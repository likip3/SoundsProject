...
public class ShiniMetalThing : MonoBehaviour
{
    private void OnTriggerEnter2D(Collider2D col)
    {
        if (!col.CompareTag("Player")) return;
        col.GetComponentInParent<SoundsSystem>().attackAngel -= 15;
        col.gameObject.GetComponentInParent<UserUI>().ShowMessage("Shaini Metal Thing", "Increase damage", true);
